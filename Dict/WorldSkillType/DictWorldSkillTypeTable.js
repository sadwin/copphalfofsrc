...
class DictWorldSkillCategoryTable extends React.Component {
        ...
        this.dataSource = new DataSource({
            store: new CustomStore({
                ...
                load: function (loadOptions) {
                    return axios.get("/work/api/dict/world-skill-type", {
                        headers: {
    ...
    render() {
        return (
            <DataGrid
                ...
                <Export enabled={true} fileName={'Типы компетенций Ворлдскиллс'} allowExportSelectedData={true}/>
                ...
                <StateStoring
                    ...
                    type={'localStorage'}
                    savingTimeout={0}
                    storageKey={'DictWorldSkillTypeTable'}/>
    ...
    handleRowDblClick(e) {
        this.props.history.push('/work/app/dict/world-skill-type/' + e.data.id);
    }

    handleAddButton() {
        axios.post("/work/api/dict/world-skill-type",
            {},
            {
                headers: {
                    'Accept': 'application/json'
            ...
            this.props.history.push("/work/app/dict/world-skill-type/" + response.data.id);
        }).catch(() => {
            notify({
                closeOnClick: true,
                closeOnOutsideClick: true,
