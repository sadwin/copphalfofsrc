...
function fetchRevisionsData(id, dispatch) {
    let revisionsUrl = "/work/api/dict/person-skill-state/" + id + "/revisions";
...
export function fetchData(id, dispatch) {
    let mainUrl = "/work/api/dict/person-skill-state/" + id;
...
export function refreshData() {
    return (dispatch, getState) => {
        let id = getState().dictPersonSkillStateView.id;
        dispatch(operationPending());
        return fetchData(id, dispatch);
    }
}