...
class DictWorldSkillStatusTable extends React.Component {
        ...
        this.dataSource = new DataSource({
            store: new CustomStore({
                ...
                load: function (loadOptions) {
                    return axios.get("/work/api/dict/person-skill-state", {
                        headers: {
    ...
    render() {
        return (
            <DataGrid
                ...
                <Export enabled={true} fileName={'Навыки'} allowExportSelectedData={true}/>
                ...
                <StateStoring
                    ...
                    type={'localStorage'}
                    savingTimeout={0}
                    storageKey={'DictPersonSkillStateTable'}/>
    ...
    handleRowDblClick(e) {
        this.props.history.push('/work/app/dict/person-skill-state/' + e.data.id);
    }

    handleAddButton() {
        axios.post("/work/api/dict/person-skill-state",
            {},
            {
                headers: {
                    'Accept': 'application/json'
            ...
            this.props.history.push("/work/app/dict/person-skill-state/" + response.data.id);
        }).catch(() => {
            notify({
                closeOnClick: true,
                closeOnOutsideClick: true,
