...
function fetchRevisionsData(id, dispatch) {
    let revisionsUrl = "/work/api/dict/world-skill-status/" + id + "/revisions";
...
export function fetchData(id, dispatch) {
    let mainUrl = "/work/api/dict/world-skill-status/" + id;
...
export function refreshData() {
    return (dispatch, getState) => {
        let id = getState().dictWorldSkillStatusView.id;
        dispatch(operationPending());
        return fetchData(id, dispatch);
    }
}