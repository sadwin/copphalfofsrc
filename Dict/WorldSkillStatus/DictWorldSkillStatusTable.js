...
class DictWorldSkillStatusTable extends React.Component {
        ...
        this.dataSource = new DataSource({
            store: new CustomStore({
                ...
                load: function (loadOptions) {
                    return axios.get("/work/api/dict/world-skill-status", {
                        headers: {
    ...
    render() {
        return (
            <DataGrid
                ...
                <Export enabled={true} fileName={'Статусы компетенций Ворлдскиллс'} allowExportSelectedData={true}/>
                ...
                <StateStoring
                    ...
                    type={'localStorage'}
                    savingTimeout={0}
                    storageKey={'DictWorldSkillStatusTable'}/>
    ...
    handleRowDblClick(e) {
        this.props.history.push('/work/app/dict/world-skill-status/' + e.data.id);
    }

    handleAddButton() {
        axios.post("/work/api/dict/world-skill-status",
            {},
            {
                headers: {
                    'Accept': 'application/json'
            ...
            this.props.history.push("/work/app/dict/world-skill-status/" + response.data.id);
        }).catch(() => {
            notify({
                closeOnClick: true,
                closeOnOutsideClick: true,
...
export default DictWorldSkillStatusTable;