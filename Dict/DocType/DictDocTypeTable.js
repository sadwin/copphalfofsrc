...
class DictDocTypeTable extends React.Component {
        ...
        this.dataSource = new DataSource({
            store: new CustomStore({
                ...
                load: function (loadOptions) {
                    return axios.get("/work/api/dict/doc-type", {
                        headers: {
    ...
    render() {
        return (
            <DataGrid
                ...
                <Export enabled={true} fileName={'Типы документов'} allowExportSelectedData={true}/>
                ...
                <Column dataField={'category'} dataType={'string'} caption={'Категория'}/>
                <Column dataField={'system'} dataType={'boolean'} caption={'Системный'}/>
    ...
    handleRowDblClick(e) {
        this.props.history.push('/work/app/dict/doc-type/' + e.data.id);
    }

    handleAddButton() {
        axios.post("/work/api/dict/doc-type",
            {},
            {
                headers: {
                    'Accept': 'application/json'
            ...
            this.props.history.push("/work/app/dict/doc-type/" + response.data.id);
        }).catch(() => {
            notify({
                closeOnClick: true,
                closeOnOutsideClick: true,
...
export default DictDocTypeTable;