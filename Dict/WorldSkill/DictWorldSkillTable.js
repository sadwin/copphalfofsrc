...
class DictMunicipalTable extends React.Component {
    ...
    render() {
        return (
            <DataGrid
                ...
                <Export enabled={true} fileName={'Компетенции Ворлдскиллс'} allowExportSelectedData={true}/>
                ...
                <StateStoring
                    ...
                    type={'localStorage'}
                    savingTimeout={0}
                    storageKey={'DictWorldSkillTable'}/>
    ...
    handleRowDblClick(e) {
        this.props.history.push('/work/app/dict/world-skill/' + e.data.id);
    }

    handleAddButton() {
        axios.post("/work/api/dict/world-skill",
            {},
            {
                headers: {
                    'Accept': 'application/json'
            ...
            this.props.history.push("/work/app/dict/world-skill/" + response.data.id);
        }).catch(() => {
            notify({
                closeOnClick: true,
                closeOnOutsideClick: true,
...
export default DictMunicipalTable;